 ...
 */
// modified by Caspar s2 2017 since the original API Demo had compile errors at current build
public class ActionBarDisplayOptions extends Activity
        implements View.OnClickListener, ActionBar.TabListener
{
   private View mCustomView;
   // Caspar: make flags an instance variable so state is remembered between clicks
   private int flags = 0;
   ...
   {
      final ActionBar bar = getActionBar();
      // Caspar: changed to instance variable to maintain state for XOR (see below)
      // int flags = 0;
      switch (v.getId())
      {
         // Caspar: use XOR operation to toggle the individual bits in the mask (old school!)
      ...
      // Caspar: add null pointer check to fix warning
      if (bar != null)
      {
         // Caspar: comment original and use different overload
